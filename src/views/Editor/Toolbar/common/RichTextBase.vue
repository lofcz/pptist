  ...
  <div class="rich-text-base">
    ...
    <ButtonGroup class="row" passive>
      <Popover trigger="click" style="width: 30%;">
        ...
        <TextColorButton first v-tooltip="LL.editor.multiStyle.textColor()" :color="richTextAttrs.color">
      ...
      <Popover trigger="click" style="width: 30%;">
        ...
        <TextColorButton v-tooltip="LL.editor.multiStyle.textHighlight()" :color="richTextAttrs.backcolor">
      ...
      <Button 
        class="font-size-btn"
        style="width: 20%;"
        v-tooltip="LL.editor.multiStyle.increaseFontSize()"
      ...
      <Button
        ...
        class="font-size-btn"
        style="width: 20%;"
        v-tooltip="LL.editor.multiStyle.decreaseFontSize()"
    ...
    <ButtonGroup class="row" passive>
      <Popover trigger="click" v-if="EXTRAS_ENABLED" v-model:value="AIPopoverVisible" style="width: 25%;">
        <template #content>
          ...
          <PopoverMenuItem center @click="execAI(AI_WRITING_COMMANDS.condense)">{{ LL.editor.richText.aiCondense() }}</PopoverMenuItem>
        </template>
        <CheckboxButton
          first
          style="width: 100%;"
          v-tooltip="LL.editor.richText.aiAssist()"
        ><span :class="{ 'ai-loading': isAIWriting }">{{ isAIWriting ? '' : 'AI' }}</span></CheckboxButton>
      </Popover>
      <CheckboxButton
        style="flex: 1;"
        v-tooltip="LL.editor.richText.clearFormat()"
        @click="emitRichTextCommand('clear')"
      ><i-icon-park-outline:format /></CheckboxButton>
      <CheckboxButton
        style="flex: 1;"
        :checked="!!textFormatPainter"
        v-tooltip="LL.editor.richText.formatPainter()"
      ...
      <Popover placement="bottom-end" trigger="click" v-model:value="linkPopoverVisible" style="width: 25%;">
        <template #content>
          <div class="link-popover">
            <Input v-model:value="link" :placeholder="LL.editor.richText.linkPlaceholder()" />
            <div class="btns">
              <Button size="small" :disabled="!richTextAttrs.link" @click="removeLink()" style="margin-right: 5px;">{{ LL.editor.richText.removeLink() }}</Button>
              <Button size="small" type="primary" @click="updateLink(link)">{{ LL.common.confirm() }}</Button>
            </div>
          </div>
        </template>
        <CheckboxButton
          ...
          v-tooltip="LL.editor.richText.hyperlink()"
    ...
    <div class="row" passive>
      <ButtonGroup style="flex: 1;">
        <Button
          ...
          style="flex: 1;"
          v-tooltip="LL.editor.richText.bulletList()"
      ...
      <ButtonGroup style="flex: 1;" passive>
        <Button
          ...
          style="flex: 1;"
          v-tooltip="LL.editor.richText.numberedList()"
    ...
    <div class="row">
      <ButtonGroup style="flex: 1;" passive>
        <Button first style="flex: 1;" v-tooltip="LL.editor.richText.decreaseIndent()" @click="emitRichTextCommand('indent', '-1')"><i-icon-park-outline:indent-left /></Button>
        <Popover trigger="click" v-model:value="indentLeftPanelVisible">
          <template #content>
            <PopoverMenuItem center @click="emitRichTextCommand('textIndent', '-1')">{{ LL.editor.richText.decreaseFirstLineIndent() }}</PopoverMenuItem>
          </template>
          <Button last class="popover-btn"><i-icon-park-outline:down /></Button>
        </Popover>
      </ButtonGroup>
      <div style="width: 10px;"></div>
      <ButtonGroup style="flex: 1;" passive>
        <Button first style="flex: 1;" v-tooltip="LL.editor.richText.increaseIndent()" @click="emitRichTextCommand('indent', '+1')"><i-icon-park-outline:indent-right /></Button>
        <Popover trigger="click" v-model:value="indentRightPanelVisible">
          <template #content>
            <PopoverMenuItem center @click="emitRichTextCommand('textIndent', '+1')">{{ LL.editor.richText.increaseFirstLineIndent() }}</PopoverMenuItem>
          </template>
          <Button last class="popover-btn"><i-icon-park-outline:down /></Button>
        </Popover>
      </ButtonGroup>
...
const updateLink = (link?: string) => {
  ...
  if (!link || !linkRegExp.test(link)) return message.error(LL.value.editor.richText.invalidLink())
...
const execAI = async (command: string) => {
  ...
  if (!content) return message.error(LL.value.editor.richText.noTextContent())
  ...
  if (typeof stream === 'object' && stream.state === -1) {
    return message.error(LL.value.editor.richText.aiConcurrencyError())
  }
